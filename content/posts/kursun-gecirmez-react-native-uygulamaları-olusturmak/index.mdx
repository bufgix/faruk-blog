---
title: Kurşun Geçirmez React Native Uygulamaları Oluşturmak
...
Öncelikle şunu söyleyebilirim sıfırdan bir uygulama geliştirip
marketlerde yayına almak sandığımdan çok daha zormuş. İlk uygulamayı yazmaya
başlarken sadece kod yazarım sonrasında hemen markete atıp kullanıcı toplarım
gibi düşündüm. Tabii ki olay öyle değilmiş. Bu başka bir yazının konusu
ama birkaç başarısız denemden sonra kendime uygun bir proje yapısı oluşturbildim.
...
[Expo]() kullanmak. Expo kullanmak yeni başlayan bir için oldukça kolay bir yol olabilir.
...
Evet bu [zımbırtı]() olmadan ben artık uygulama _yazmıyorum_. Ilerde anlatacağım coğu şeyde Typescript
yardımını kullandığım için Typescriptin olmaması kabul edilemez. Unutmayın amacımız kurşun geçirmeyen,
gelir getirmesi istenen, sürdürülebilir bir uygulama oluşturmak. Typescript bu konuda baya kolaylik sağlıyor.
...
- `assets` burada uygulamada kullancak resimler ve fontlar gibi static dosyalar bulunur
- `components` burada uygulama bileşenleri bulunur
  > Tercihen ben bu dizin içinde bir de `index.ts` ekleyip tüm componentleri buradan exportluyorum. Bu bana komponenti
...
- `bootstrap.tsx` Bu dosya uygulamamızın kök komponenti. Genelde burada uygulama açılmadan önce yüklencek
  asenkron işlemler(network requestleri, local databaseden inital verilerin çekilmesi vb.) yapılır. Eğer
  SplashScreen kullancaksınız bütün asenkron işlemleri yaptıktan sonra Splash ekranını kaldırabilirsiniz.
...
Yaptığım uygulamaların hiçbirinde tasarımı ben yapmadım elimde her zaman bir [Figma]() dosyası oldu. Figmada
iconları `.svg` olarak export alabildiğimden kolayca React Native içerisinde kullanbiliyordum.

Bunun için kullandığım araç [SVGR](). SVGR nin amacı, svg dosyalarını benim için React componentlerine
dönüştürmek.
...
React Native'e vermek çok mantıklı değil. Çünkü faklı ekranlarda aynı pixel değeri istediğimiz bir şey
değildir. Bunu çözmek için kullandığım yöntem ise şöyle:
...
```tsx:title=/views/Cars/index.tsx {6-18}
...
Görüldüğü gibi her sayfada en az 3 tane state değişkeni sürekli olarak
tanımlamamız gerekiyor. Bunu daha efektif bir hale getirebiliriz.

React tarafında backend servisinden veri çekerken [React Query]() kullanıyorum.
Basit olarak şöyle

```tsx:title=/views/Cars/index.tsx {8-10}
...
Ben global state için uygulamarımda [Redux]() ve [Redux-Toolkit]() kullanıyorum. Reduxdan önce state için [Mobx](),
[Mobx-State-Tree]() gibi kütüphaneler de kullandım. Redux o zamanlar hep bana bir sürü reducer yazdığın
...
Store yazarken dikkta ettiğim 2 şey var. Biri store'u typescript ile doğru ayarlamak.
...
### Uygulama için satın alma & Abonelik
